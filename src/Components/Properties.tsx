...
useEffect(() => {
  axios.get('http://localhost:7070/api/new-properties')
  .then((response) => {
      ...
      <section className="mt-6 pt-4 pb-5  ">
        ...
        <div className="grid md:grid-cols-3  md:m-6 md:px-8 p-4 gap-x-10 gap-y-12 place-items-center">
          ...
          {properties.map((item, id) => (
            <div key={id} className=" shadow hover:-translate-y-1 hover:scale-110  duration-300 rounded-md">
            ...
            >
                <h4 className=" py-2 text-sky-900 font-bold hover:text-orange-600">{item.name}</h4>
